...
import { uploadEncryptedSend, type SendUploadPart, type SendUploadPartUrl } from "./upload";

vi.mock("@stenvault/aead-stream", () => ({
  encryptSendChunk: vi.fn(async (chunk: Uint8Array) => {
...
describe("uploadEncryptedSend", () => {
  ...
  it("skips PUT for prefilled parts but still populates chunkHashes and returns ETags", async () => {
    const key = await setupKey();
    // 5 parts total. Parts 1, 2, 4 are prefilled; 3 and 5 must be PUT.
    const prefilledParts: SendUploadPart[] = [
      { partNumber: 1, etag: "prefilled-1" },
      { partNumber: 2, etag: "prefilled-2" },
      { partNumber: 4, etag: "prefilled-4" },
    ];

    const result = await uploadEncryptedSend({
      fileBlob: new Blob([new Uint8Array(80)]),
      key,
      baseIv: new Uint8Array(12),
      fileIndex: 0,
      initialPartUrls: urlsFor(5),
      totalParts: 5,
      refreshPartUrls: noopRefresh(),
      abortSignal: { aborted: false },
      onProgress: () => {},
      onSpeed: () => {},
      skipPartNumbers: [1, 2, 4],
      prefilledParts,
    });

    // Only parts 3 and 5 should have been PUT to R2.
    expect(MockXHR.callLog.map((c) => c.url).sort()).toEqual([
      "https://r2.example.test/part-3",
      "https://r2.example.test/part-5",
    ]);

    // Final parts list carries prefilled ETags for skipped indices.
    const byPart = new Map(result.parts.map((p) => [p.partNumber, p.etag]));
    expect(byPart.get(1)).toBe("prefilled-1");
    expect(byPart.get(2)).toBe("prefilled-2");
    expect(byPart.get(4)).toBe("prefilled-4");
    expect(byPart.get(3)).toBe(`"mock-etag"`);
    expect(byPart.get(5)).toBe(`"mock-etag"`);

    // Manifest must be dense — every slot populated, including skipped parts.
    expect(result.chunkHashes).toHaveLength(5);
    expect(result.chunkHashes.every((h) => typeof h === "string" && h.length > 0)).toBe(true);
  });

  it("issues zero PUTs when every part is prefilled", async () => {
    const key = await setupKey();
    const prefilledParts: SendUploadPart[] = [
      { partNumber: 1, etag: "pf-1" },
      { partNumber: 2, etag: "pf-2" },
      { partNumber: 3, etag: "pf-3" },
    ];
    ...
    const result = await uploadEncryptedSend({
      ...
      onProgress: () => {},
      onSpeed: () => {},
      skipPartNumbers: [1, 2, 3],
      prefilledParts,
    });

    expect(MockXHR.callLog).toEqual([]);
    expect(result.parts.map((p) => p.partNumber)).toEqual([1, 2, 3]);
    expect(result.parts.map((p) => p.etag)).toEqual(["pf-1", "pf-2", "pf-3"]);
    expect(result.chunkHashes).toHaveLength(3);
    expect(result.chunkHashes.every((h) => typeof h === "string" && h.length > 0)).toBe(true);
  });

  it("throws when skipPartNumbers and prefilledParts lengths disagree", async () => {
    const key = await setupKey();
    await expect(
      uploadEncryptedSend({
        fileBlob: blobOf([1, 2, 3]),
  ...
  it("throws when skipPartNumbers references a part with no prefilled ETag", async () => {
    const key = await setupKey();
    await expect(
      uploadEncryptedSend({
        fileBlob: new Blob([new Uint8Array(32)]),
        ...
        onSpeed: () => {},
        skipPartNumbers: [2],
        prefilledParts: [{ partNumber: 1, etag: "wrong-part" }],
      }),
    ).rejects.toThrow(/no matching prefilledParts ETag/);
  });
