 ...
 * - Supports resume via `skipPartNumbers` + `prefilledParts`. Skipped parts
 *   are still encrypted + hashed locally (the manifest is dense), but the
 *   PUT is omitted. This only works because V2 derived-IV chunk encryption
 *   is deterministic — the ciphertext we skip sending is byte-identical to
 *   what R2 already has. That property is pinned by
 *   `packages/aead-stream/src/chunk.test.ts` ("is deterministic: re-encrypting
 *   the same chunk produces byte-identical ciphertext"); any change that
 *   breaks determinism breaks resume, so the test is the early-warning.
 */
import { encryptSendChunk, hashEncryptedChunk } from "@stenvault/aead-stream";
...
export interface UploadEncryptedSendParams {
    ...
    key: CryptoKey;
    baseIv: Uint8Array;
    /** Position of this file in the bundle. Threaded into the per-chunk IV
     *  so chunks across different files never share an IV under the same
     *  session key. */
    fileIndex: number;
    /** Initial batch of URLs from `initiateBundle`. May be shorter than `totalParts`. */
    ...
    /**
     * Part numbers already uploaded to R2 (from `queryUploadStatus`). For
     * these, we still re-encrypt and hash locally — V2 derived-IV makes
     * the ciphertext byte-identical, so the hash lands in the same slot
     * of `chunkHashes` the manifest signs over — but we skip the PUT.
     * Must line up with `prefilledParts` exactly.
     */
    skipPartNumbers?: ReadonlyArray<number>;
    /**
     * Server-side ETags for the `skipPartNumbers`. One entry per skipped
     * part; the final `parts` result uses these verbatim instead of an
     * ETag we would have harvested from a PUT response.
     */
...
export async function uploadEncryptedSend(
    ...
    // Resolve the skip set: either both args are absent or both must line up.
    // A mismatch is a caller bug — fail fast rather than silently dropping
    // ETags (which would poison `completeSend`).
    const skipSet = new Set<number>();
    const prefilledByPart = new Map<number, string>();
    if (skipPartNumbers || prefilledParts) {
        const nums = skipPartNumbers ?? [];
        const etags = prefilledParts ?? [];
    ...
    // Seed with what R2 already has so the bar doesn't climb from zero while
    // the client re-encrypts chunks it won't re-upload. With concurrency 4
    // the single-file path usually finishes the skipped chunks fast enough
    // that the difference is barely visible, but seeding matches the
    // streaming-zip path and keeps resume UX consistent between them.
    let bytesCompleted = Math.min(fileBlob.size, skipSet.size * SEND_PART_SIZE);
    ...
    const tasks = Array.from({ length: totalParts }, (_, i) => async () => {
        ...
        // Re-encrypt + hash even for skipped parts: the manifest downstream
        // signs `chunkHashes[partIndex]` for every index, and server-side
        // integrity verification re-hashes from R2 storage. Derived-IV
        // (fileIndex + chunkIndex) means the ciphertext we throw away
        // here is the same one already in R2 for a prefilled part.
        const encrypted = await encryptSendChunk(chunk, key, baseIv, fileIndex, partIndex);
        const chunkHash = await hashEncryptedChunk(encrypted);
        chunkHashes[partIndex] = chunkHash;

        if (skipSet.has(partNumber)) {
            const prefilledEtag = prefilledByPart.get(partNumber)!;
            partEtags[partIndex] = prefilledEtag;
            // bytesCompleted was pre-seeded with the full skip set at init,
            // so don't double-count. Still refresh speed + progress so the UI
            // stays responsive while the producer burns through skipped parts.
            const remaining = fileBlob.size - bytesCompleted;
            updateSpeed(chunk.byteLength, remaining);
            emitProgress();

            completedPartEntries.push({ partNumber, etag: prefilledEtag });
