...
describe("publicSendCrypto", () => {
    ...
    describe("encryptMetadata / decryptMetadata (V2 BundleManifest)", () => {
        it("roundtrips a single-file bundle manifest", async () => {
            const key = await generateSendKey();
            const manifest = {
                v: 2 as const,
                files: [{ fileIndex: 0, name: "report.pdf", size: 12345, type: "application/pdf" }],
            };

            const { ciphertext, iv } = await encryptMetadata(manifest, key);
        ...
        it("roundtrips a multi-file bundle manifest", async () => {
            const key = await generateSendKey();
            const manifest = {
                v: 2 as const,
                files: [
                    { fileIndex: 0, name: "a.txt", size: 1, type: "text/plain" },
                    { fileIndex: 1, name: "b.png", size: 2048, type: "image/png" },
                    { fileIndex: 2, name: "c.pdf", size: 99999, type: "application/pdf" },
                ],
            };
        ...
        it("handles unicode filenames", async () => {
            ...
            const manifest = {
                v: 2 as const,
                files: [
                    { fileIndex: 0, name: "relatório_2026.pdf", size: 1, type: "application/pdf" },
                ],
            };
        ...
        it("fails with wrong key", async () => {
            ...
            const manifest = {
                v: 2 as const,
                files: [{ fileIndex: 0, name: "test.txt", size: 1, type: "text/plain" }],
            };

            const { ciphertext, iv } = await encryptMetadata(manifest, key1);
